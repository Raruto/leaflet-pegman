...
*/
L.Control.Pegman=L.Control.extend({includes:L.Evented?L.Evented.prototype:L.Mixin.Events,options:{position:"bottomright",theme:"leaflet-pegman-v3-default",debug:false,apiKey:"",libraries:"",mutant:{attribution:'Map data: &copy; <a href="https://www.google.com/intl/en/help/terms_maps.html">Google</a>',pane:"overlayPane",type:null},pano:{enableCloseButton:true}},__interactURL:"https://unpkg.com/interactjs@1.2.9/dist/interact.min.js",__gmapsURL:"https://maps.googleapis.com/maps/api/js?v=3",__mutantURL:"https://unpkg.com/leaflet.gridlayer.googlemutant@0.8.0/Leaflet.GoogleMutant.js",
initialize:function(options){if(typeof options.logging!=="undefined")options.debug=options.logging;L.Util.setOptions(this,options);this._mousePos={direction:{},old:{}};this._pegmanMarkerCoords=null;this._streetViewCoords=null;this._streetViewLayerEnabled=false;this._dropzoneMapOpts={accept:".draggable",overlap:.75,ondropactivate:L.bind(this.onDropZoneActivated,this),ondragenter:L.bind(this.onDropZoneDragEntered,this),ondragleave:L.bind(this.onDropZoneDragLeaved,this),ondrop:L.bind(this.onDropZoneDropped,
this),ondropdeactivate:L.bind(this.onDropZoneDeactivated,this)};this._draggableMarkerOpts={inertia:false,onmove:L.bind(this.onDraggableMove,this),onend:L.bind(this.onDraggableEnd,this)};this._pegmanMarkerOpts={draggable:true,icon:L.icon({className:"pegman-marker",iconSize:[52,52],iconAnchor:[26,13],iconUrl:"data:image/png;base64,"+"iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAAFElEQVR4XgXAAQ0AAABAMP1L30IDCPwC/o5WcS4AAAAASUVORK5CYII="})};this._lazyLoaderAdded=false},onAdd:function(map){this._map=
map;this._container=L.DomUtil.create("div","leaflet-pegman pegman-control leaflet-bar");this._pegman=L.DomUtil.create("div","pegman draggable drag-drop",this._container);this._pegmanButton=L.DomUtil.create("div","pegman-button",this._container);this._pegmanMarker=L.marker([0,0],this._pegmanMarkerOpts);this._panoDiv=L.DomUtil.create("div","pano-canvas",this._map._container);L.DomUtil.addClass(this._map._container,this.options.theme);L.DomEvent.disableClickPropagation(this._panoDiv);L.DomEvent.on(this._container,
"click mousedown dblclick",this._disableClickPropagation,this);this._container.addEventListener("mousedown",this._loadScripts.bind(this,true),false);this._container.addEventListener("touchstart",this._loadScripts.bind(this,true),false);this._container.addEventListener("mouseover",this._loadScripts.bind(this,false),false);this._loadInteractHandlers();this._loadGoogleHandlers();L.DomEvent.on(document,"mousemove",this.mouseMoveTracking,this);L.DomEvent.on(document,"keyup",this.keyUpTracking,this);this._pegmanMarker.on("dragend",
this.onPegmanMarkerDragged,this);this._map.on("click",this.onMapClick,this);this._map.on("layeradd",this.onMapLayerAdd,this);return this._container},onRemove:function(map){this._googleStreetViewLayer.remove();this._pegmanMarker.remove();L.DomUtil.remove(this._panoDiv);L.DomEvent.off(document,"mousemove",this.mouseMoveTracking,this);L.DomEvent.off(document,"keyup",this.keyUpTracking,this)},_log:function(args){if(this.options.debug)console.log(args)},_addClasses:function(el,classNames){classNames=classNames.split(" ");
for(var s in classNames)L.DomUtil.addClass(el,classNames[s])},_removeClasses:function(el,classNames){classNames=classNames.split(" ");for(var s in classNames)L.DomUtil.removeClass(el,classNames[s])},_removeAttributes:function(el,attrNames){for(var a in attrNames)el.removeAttribute(attrNames[a])},_insertAfter:function(targetNode,newNode){targetNode.parentNode.insertBefore(newNode,targetNode.nextSibling)},_translateElement:function(el,dx,dy){if(dx===false&&dy===false)this._removeAttributes(this._pegman,
...
-1)script.src=url.replace("callback=?","callback="+jsonp)+query.slice(0,-1);else script.src=url+query+"callback="+jsonp;var loaded=function(){if(!this.readyState||this.readyState==="loaded"||this.readyState==="complete"){script.onload=script.onreadystatechange=null;if(script&&script.parentNode)script.parentNode.removeChild(script)}};script.async=true;script.onload=script.onreadystatechange=loaded;var head=document.head||document.getElementsByTagName("head")[0]||document.documentElement;head.insertBefore(script,
head.firstChild)}});L.control.pegman=function(options){return new L.Control.Pegman(options)};
